...
/**
 * A bounded {@link BlockingQueue} that uses the same ordering rules as
 * {@link PriorityBlockingQueue} and supplies blocking operations similar to
 * {@link ArrayBlockingQueue}. This class does not permit {@code null} elements. A priority queue
 * relying on {@link Comparable} natural ordering also does not permit insertion of non-comparable
 * objects (doing so results in {@code ClassCastException}). However, relying on object natural
 * order is not necessary if upon queue construction a {@link Comparator} is provided which is used
 * to order the priority queue.<br/>
 * <br/>
 * This queue implementation is a classic bounded buffer, in which a fixed-sized array holds
 * elements inserted by producers and extracted by consumers in priority order. Once created, the
 * capacity cannot be increased. Attempts to e.g. {@link #put(Object)} an element into a full queue
 * will result in the operation blocking; attempts to e.g. {@link #take()} an element from an empty
 * queue will similarly block.<br/>
 * <br/>
 ...
 * set to true grants threads access in FIFO order. Fairness generally decreases throughput but
 * reduces variability and avoids thread starvation.<br/>
 * <br/>
 * This class and its iterator implement all of the methods of the {@link Collection} and
 * {@link Iterator} interfaces. The Iterator provided in method {@link #iterator()} is NOT
 * guaranteed to traverse the elements of the {@link BoundedPriorityBlockingQueue} in any particular
 * order. Also, method {@code #drainTo} can be used to remove some or all elements in priority order
 * and place them in another collection.<br/>
 * <br/>
 * Operations on this class make no guarantees about the ordering of elements with equal priority.
 * If this behavior is desired it is required to enforce an ordering, by e.g. defining custom
 * classes or comparators that use a secondary key to break ties in primary priority values.<br/>
 * <br/>
 *
...
public class BoundedPriorityBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
     ...
     * descendant d of n, n <= d. The element with the lowest value is in queue[
     * {@link #TREE_ROOT_INDEX}], assuming the queue is nonempty.
     */
    ...
    /**
     * Creates a {@code BoundedPriorityBlockingQueue} with the default access policy and the
     * specified capacity that orders its elements according to their {@link Comparable} natural
     * ordering.
     *
    ...
    /**
     * Creates a {@code BoundedPriorityBlockingQueue} with the specified access policy and the
     * capacity that orders its elements according to their {@link Comparable} natural ordering.
     *
     * @param capacity The capacity for this priority queue.
    ...
    /**
     * Creates a {@code BoundedPriorityBlockingQueue} with the default access policy and the
     * specified capacity that orders its elements according to the specified {@link Comparator}.
     *
     * @param capacity The capacity for this priority queue.
     * @param comparator The {@link Comparator} that will be used to order this priority queue. If
     *        {@code null}, the {@link Comparable} natural ordering of the elements will be used.
     * @throws IllegalArgumentException If {@code capacity} is less than 1.
     */
    ...
    /**
     * Creates an {@link BoundedPriorityBlockingQueue} with the provided capacity, the specified
     * access policy and initially containing the elements of the provided collection.
     *
     * @param capacity The capacity of this queue.
     * @param fair If {@code true} then queue accesses for threads blocked on insertion or removal,
     *        are processed in FIFO order; if {@code false} the access order is unspecified.
     * @param collection The collection of elements to initially contain.
     * @throws IllegalArgumentException If {@code capacity} is less than 1, or less than the
     *         provided collection's size.
     ...
     * exceeding the queue's capacity, returning {@code true} upon success and throwing an
     ...
     * @throws InterruptedException If thread is interrupted while waiting for an element to become
     *         available.
     */
    @Override
    ...
    /**
     * Inserts the specified element into this queue, waiting for space to become available if the
     * queue is full.
     *
     * @param element The element to add.
    ...
    /**
     * Inserts the specified element into this queue if it is possible to do so immediately without
     * exceeding the queue's capacity, returning {@code true} upon success and {@code false} if this
     * queue is full. This method is generally preferable to {@link #add}, which can fail to insert
    ...
    /**
     * Inserts the specified element into this queue, waiting up to the specified wait time for
     ...
     * memory or resource constraints) accept without blocking. This is always equal to the capacity
     * of this queue minus the current {@link #size()} of this queue.<br/>
     * <br/>
     * Note that you cannot always tell if an attempt to insert an element will succeed by
     * inspecting {@link #remainingCapacity()} because it may be the case that another thread is
     * about to insert or remove an element.
     *
     * @return The number of additional elements that this queue can ideally accept without
     *         blocking.
     */
    @Override
     ...
     * such an element. Returns {@code true} if this queue contained the specified element (or
     * equivalently, if this queue changed as a result of the call).<br/>
     * <br/>
     * Removal of interior elements in priority based queues is an intrinsically slow and disruptive
     * operation, so should be undertaken only in exceptional circumstances, ideally only when the
     * queue is known not to be accessible by other threads.
     *
     ...
     * operation may be more efficient than repeatedly polling this queue. A failure encountered
     * while attempting to add elements to the provided collection may result in elements being in
     * neither, either or both collections when the associated exception is thrown. Attempts to
     * drain a queue to itself result in {@link IllegalArgumentException}. Further, the behavior of
     * this operation is undefined if the provided collection is modified while the operation is in
     * progress.
     *
     * @param collectionToDrainTo The collection to transfer elements into.
     * @return The number of elements transferred.
     * @throws UnsupportedOperationException If addition of elements is not supported by the
     ...
     * @throws IllegalArgumentException If the specified collection is this queue, or some property
     *         of an element of this queue prevents it from being added to the specified collection.
     */
    @Override
    ...
    /**
     * Removes at most the provided number of available elements from this queue and adds them to
     * the provided collection. A failure encountered while attempting to add elements to the
     * provided collection may result in elements being in neither, either or both collections when
     * the associated exception is thrown. Attempts to drain a queue to itself result in
     * {@link IllegalArgumentException}. Further, the behavior of this operation is undefined if the
     * provided collection is modified while the operation is in progress.
     *
     * @param collectionToDrainTo The collection to transfer elements into.
    ...
    public int drainTo(Collection<? super E> collectionToDrainTo, int maxElementsToDrain) {
        ...
        try {
            ...
            while (numberOfDrainedElements < maxElementsToDrain
                    && (drainedElement = extractElementWithoutSignalingWaitingThread()) != null) {
                collectionToDrainTo.add(drainedElement);
                ++numberOfDrainedElements;
            }
     ...
     * the returned array without impacting this queue.
     *
     ...
     * of this queue. If this queue fits in the specified array with room to spare (i.e., the array
     * has more elements than this queue), the element in the array immediately following the end of
     * the queue is set to null.<br/>
     * <br/>
     * Note that {@code toArray(new Object[0])} is identical in function to {@code toArray()}.
     *
     * @param newArray The array into which the elements of the queue are to be stored, if it is big
     ...
     * elements in any particular order.<br/>
     * <br/>
    ...
    /**
     * Verifies that the provided object is not null, and if it is a {@link NullPointerException} is
     * thrown.
     *
    ...
    /**
     * Verifies that the two provided references don't refer to the very same object, and if they do
    ...
    /**
     * Extracts an element from the queue (if present), and if successful in doing so the method
     * will signal an awaiting thread that an element has been removed.<br/>
     * <br/>
     * Call only using mutual exclusion.<br/>
     * <br/>
    ...
    /**
     * Extracts an element from the queue (if present). This method will NOT signal an awaiting
     * thread that an element has been removed.<br/>
     * <br/>
     * Call only using mutual exclusion.<br/>
     * <br/>
     * This method will return null in case the queue is empty.
    ...
    /**
     * Inserts the provided element at the insertion index, maintaining heap invariant by promoting
     * the element up the tree until it is greater than or equal to its parent, or is the root of
     * the tree.<br/>
     * <br/>
     * This method does not guarantee any type of thread-safety - such mechanisms are up to the call
     * site.<br/>
     * <br/>
     * This method is static, with heap state as arguments.
     *
     * @param insertionIndex The position to insert at, which might change when traversing the heap
     *        searching for the actual insertion index.
     * @param element The element to insert.
     * @param heap The heap array.
     * @param comparator The {@link Comparator} to use. Null value is supported, and in that case
     *        the {@link Comparable} natural ordering of the provided element is used.
     */
    private static <T> void siftUpElement(int insertionIndex, T element, Object[] heap, Comparator<? super T> comparator) {
        ...
        while (insertionIndex > 0) {
            ...
            boolean equalOrGreaterThanZero =
                    hasComparator ? (comparator.compare(element, parentElement) >= 0) : comparableElement
                            .compareTo(parentElement) >= 0;
    ...
    /**
     * Inserts provided element at the insertion index, maintaining heap invariant by demoting the
     * element down the tree repeatedly until it is less than or equal to its children or is a leaf.<br/>
     * <br/>
     * This method does not guarantee any type of thread-safety - such mechanisms are up to the call
     * site.<br/>
     * <br/>
     ...
     *        the {@link Comparable} natural ordering of the provided element is used.
     */
    private static <T> void siftDownElement(int insertionIndex, T element, Object[] heap, int heapSize,
        ...
        while (insertionIndex < half) {
            ...
            if (rightIndex < heapSize) {
                boolean greaterThanZero =
                        hasComparator ? comparator.compare((T) childElement, (T) heap[rightIndex]) > 0
            ...
            boolean equalOrLessThanZero =
                    hasComparator ? comparator.compare(element, (T) childElement) <= 0 : comparableElement
                            .compareTo((T) childElement) <= 0;
    ...
    private final class QueueIterator implements Iterator<E> {
        ...
        /**
         * Identity-based removal of the provided object. This method will be executed using mutual
         * exclusion.
         */
